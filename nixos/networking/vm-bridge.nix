/**
  Module to help with developing in VMs.  Sets up a bridge network and
  domain name for a qemu VM.

  qemu networking configuration for this setup:

  - -device virtio-net,netdev=n1
  - -netdev bridge,id=n1,br=br0,helper=$(which qemu-bridge-helper)

  Also set up a static IP address of 192.168.9.2/24, with a gateway of
  192.168.9.1.
*/
{ pkgs, ... }:
let
