# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  bauer = {
    pname = "bauer";
    version = "v2.0.1";
    src = fetchFromGitHub {
      owner = "matthewbauer";
      repo = "bauer";
      rev = "v2.0.1";
      fetchSubmodules = false;
      sha256 = "sha256-jenFLJeVX1OlckvCv+c/u2GG8UNQatfQ8OwPu1cs4Q8=";
    };
  };
  deepfilternet = {
    pname = "deepfilternet";
    version = "v0.5.6";
    src = fetchFromGitHub {
      owner = "Rikorose";
      repo = "DeepFilterNet";
      rev = "v0.5.6";
      fetchSubmodules = false;
      sha256 = "sha256-5bYbfO1kmduNm9YV5niaaPvRIDRmPt4QOX7eKpK+sWY=";
    };
  };
  drivestrike = {
    pname = "drivestrike";
    version = "2.1.22-31";
    src = fetchurl {
      url = "https://app.drivestrike.com/static/yum/drivestrike.rpm";
      sha256 = "sha256-2O0TjRhuwLd+QPUxV9tHeuWYtGoRnBa6icU7DMmxWyI=";
    };
  };
  edopro = {
    pname = "edopro";
    version = "2eea2b7e92db7bdc6d3746373202e145fbe00410";
    src = fetchFromGitHub {
      owner = "edo9300";
      repo = "edopro";
      rev = "2eea2b7e92db7bdc6d3746373202e145fbe00410";
      fetchSubmodules = true;
      sha256 = "sha256-524ZLBwJ6wv69wP3upbmTn99igtMfzKuPmnPYD+xw8c=";
    };
    date = "2025-02-21";
  };
  edopro-assets = {
    pname = "edopro-assets";
    version = "40.1.4";
    src = fetchTarball {
      url = "https://github.com/ProjectIgnis/edopro-assets/releases/download/40.1.4/ProjectIgnis-EDOPro-40.1.4-linux.tar.gz";
      sha256 = "sha256-vZhkWJ1ZoNEwdc5kM1S0hyXnWmupiTOanCi9DCuqw/k=";
    };
  };
  edopro-irrlicht = {
    pname = "edopro-irrlicht";
    version = "c869c15209e5151f44a8606c04d2028abb0ac73c";
    src = fetchFromGitHub {
      owner = "edo9300";
      repo = "irrlicht1-8-4";
      rev = "c869c15209e5151f44a8606c04d2028abb0ac73c";
      fetchSubmodules = false;
      sha256 = "sha256-Xq+YPY8ekodfeXJL4cHcowfDn5ainuJ9Wn9zga1ibLM=";
    };
    date = "2025-02-09";
  };
  eglot-x = {
    pname = "eglot-x";
    version = "5b5a56ddf8a087520f5e25a7343ec6503338e35b";
    src = fetchFromGitHub {
      owner = "nemethf";
      repo = "eglot-x";
      rev = "5b5a56ddf8a087520f5e25a7343ec6503338e35b";
      fetchSubmodules = false;
      sha256 = "sha256-6FY/Y5ahA82N0uJ0Dvb5e390UoVBEi8E8kKGfyknM/k=";
    };
    date = "2025-02-16";
  };
  firefox-ui-fix = {
    pname = "firefox-ui-fix";
    version = "v8.6.6";
    src = fetchFromGitHub {
      owner = "black7375";
      repo = "Firefox-UI-Fix";
      rev = "v8.6.6";
      fetchSubmodules = false;
      sha256 = "sha256-9pNphWnxiIe0bCpPdNQk/7y/zPvFKBM/djQoED+PJJ0=";
    };
  };
  gauth = {
    pname = "gauth";
    version = "v1.5.0";
    src = fetchFromGitHub {
      owner = "pcarrier";
      repo = "gauth";
      rev = "v1.5.0";
      fetchSubmodules = false;
      sha256 = "sha256-t5hvbJ8Oqqq52SHFxl6jmNOTC+6fQOJ+jyWoQyY008Q=";
    };
  };
  gcs = {
    pname = "gcs";
    version = "v5.27.0";
    src = fetchFromGitHub {
      owner = "richardwilkes";
      repo = "gcs";
      rev = "v5.27.0";
      fetchSubmodules = false;
      sha256 = "sha256-N4a+7a1XpDXsZTDiTh1AZIzXfk/A+gV4uBZdGZh+S5I=";
    };
  };
  ohmyzsh = {
    pname = "ohmyzsh";
    version = "6e7ac0544e71c7b777746cb50f70de68c6495b86";
    src = fetchFromGitHub {
      owner = "ohmyzsh";
      repo = "ohmyzsh";
      rev = "6e7ac0544e71c7b777746cb50f70de68c6495b86";
      fetchSubmodules = false;
      sha256 = "sha256-d4w15ja5qe4axOwNyIoXZdVcgUedAIgZQrhINmNNJFw=";
    };
    date = "2025-02-19";
  };
  phosphor-icons = {
    pname = "phosphor-icons";
    version = "b8f00b5ad3d91471afec6a81c69154cf87e6e233";
    src = fetchFromGitHub {
      owner = "phosphor-icons";
      repo = "web";
      rev = "b8f00b5ad3d91471afec6a81c69154cf87e6e233";
      fetchSubmodules = false;
      sha256 = "sha256-hio7aUN1tg5qXHNqi9PzbwBOPrFD9jnPA2/qXJ8Li8I=";
    };
    date = "2025-01-30";
  };
  tridactyl-emacs = {
    pname = "tridactyl-emacs";
    version = "ee82f3de875e7cda518cfd2c469ec21ed94c3cfb";
    src = fetchFromGitHub {
      owner = "jumper047";
      repo = "tridactyl_emacs_config";
      rev = "ee82f3de875e7cda518cfd2c469ec21ed94c3cfb";
      fetchSubmodules = false;
      sha256 = "sha256-OGSVKUoZ9E5yqJQfTnfx6QW4vLbdQ5/V3VX65TuQG4k=";
    };
    date = "2023-12-22";
  };
}
