# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub }:
{
  bauer = {
    pname = "bauer";
    version = "v1.5.3";
    src = fetchFromGitHub ({
      owner = "matthewbauer";
      repo = "bauer";
      rev = "v1.5.3";
      fetchSubmodules = false;
      sha256 = "sha256-IBzX7WASluoxXVdWkoJHRIQQF4Fi7IJOvRfRl+W3YZI=";
    });
  };
  firefox-ui-fix = {
    pname = "firefox-ui-fix";
    version = "eec36701e60aa20ac2548e76aac4ef6f41889563";
    src = fetchFromGitHub ({
      owner = "black7375";
      repo = "Firefox-UI-Fix";
      rev = "eec36701e60aa20ac2548e76aac4ef6f41889563";
      fetchSubmodules = false;
      sha256 = "sha256-vUuGsRwBs+tGhfaa4mjIVx/R5JQp6kT+Y07xtv8HQc8=";
    });
  };
  gauth = {
    pname = "gauth";
    version = "v1.2.2";
    src = fetchFromGitHub ({
      owner = "pcarrier";
      repo = "gauth";
      rev = "v1.2.2";
      fetchSubmodules = false;
      sha256 = "sha256-EprxybxUEvpT4jUaI75Y4E3nzskjX8HA9qJap6/Ws+0=";
    });
  };
  ohmyzsh = {
    pname = "ohmyzsh";
    version = "b4f9698733d7b29cc495e649e26fd6c3a5dcfcae";
    src = fetchFromGitHub ({
      owner = "ohmyzsh";
      repo = "ohmyzsh";
      rev = "b4f9698733d7b29cc495e649e26fd6c3a5dcfcae";
      fetchSubmodules = false;
      sha256 = "sha256-yvsNYoptmmm3BJeOdyt7zGuayYroraiiumOio9eZZ74=";
    });
  };
  phosphor-icons = {
    pname = "phosphor-icons";
    version = "d3b385a38acbb8b16a068e8e2515660af6978cf9";
    src = fetchFromGitHub ({
      owner = "phosphor-icons";
      repo = "web";
      rev = "d3b385a38acbb8b16a068e8e2515660af6978cf9";
      fetchSubmodules = false;
      sha256 = "sha256-zq7z2IEH6Cfv/3O1jYe1MYbvZKb+FicGf4QFDlbseTI=";
    });
  };
  stumpwm = {
    pname = "stumpwm";
    version = "05ec1a92bd4d4a81f2dff023cbf9916982157a6d";
    src = fetchFromGitHub ({
      owner = "stumpwm";
      repo = "stumpwm";
      rev = "05ec1a92bd4d4a81f2dff023cbf9916982157a6d";
      fetchSubmodules = false;
      sha256 = "sha256-gOtYR5W5HZgknTG+jiUyZK9k3vnSkvAGfs9i2h4tiAw=";
    });
  };
  stumpwm-contrib = {
    pname = "stumpwm-contrib";
    version = "36daccd715e1cc6c1badab7cd87e34a8514f3b6b";
    src = fetchFromGitHub ({
      owner = "stumpwm";
      repo = "stumpwm-contrib";
      rev = "36daccd715e1cc6c1badab7cd87e34a8514f3b6b";
      fetchSubmodules = false;
      sha256 = "sha256-oW01W3ICwFdnDtO6y4aWXDIthw9iWU+lntNGcS80TXU=";
    });
  };
  tridactyl-emacs = {
    pname = "tridactyl-emacs";
    version = "f0b84500117bde9bf008fb25c264aa3386b6c07f";
    src = fetchFromGitHub ({
      owner = "jumper047";
      repo = "tridactyl_emacs_config";
      rev = "f0b84500117bde9bf008fb25c264aa3386b6c07f";
      fetchSubmodules = false;
      sha256 = "sha256-+F5ENKOJXnUCH2kEFARhc2ieChUXNn0IIdCX/cjUgHE=";
    });
  };
  zsh-background-notify = {
    pname = "zsh-background-notify";
    version = "d5f0430cb052f82c433c17707816910da87e201e";
    src = fetchFromGitHub ({
      owner = "t413";
      repo = "zsh-background-notify";
      rev = "d5f0430cb052f82c433c17707816910da87e201e";
      fetchSubmodules = false;
      sha256 = "sha256-NhDAOKCkNItODP8d68FVySZghTvPKXG2eBPlvkCZDl0=";
    });
  };
}
